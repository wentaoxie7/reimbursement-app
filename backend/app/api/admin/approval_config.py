...
def list_sequences(
    user: User = Depends(require_page_access("ADMIN_APPROVAL")),
    ...
    return [ApprovalSequenceResponse.model_validate(sequence) for sequence in items]
...
def create_sequence(
    ...
    db.commit()
    db.refresh(sequence)
    return ApprovalSequenceResponse.model_validate(sequence)
...
def set_default_sequence(
    ...
    sequence = db.get(ApprovalSequence, sequence_id)
    if not sequence or sequence.org_id != user.org_id or not sequence.active:
...
def delete_sequence(
    ...
    if sequence.is_default and not replacements:
        raise HTTPException(
